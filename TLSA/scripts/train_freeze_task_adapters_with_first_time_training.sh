...
identifire=peft_with_ewc_${epochs}_${importance}_enl_${encoder_ids_for_lang}_ent_${encoder_ids_for_task}_del_${decoder_ids_for_lang}_det_${decoder_ids_for_task}_freeze_task_adapters_after_first_time_training
...
CUDA_VISIBLE_DEVICES=$gpu_ids accelerate launch --main_process_port $port_no main.py \
    ...
    --freeze_task_adapters_after_first_language > $output_dir/training_logs/${seq}.log
