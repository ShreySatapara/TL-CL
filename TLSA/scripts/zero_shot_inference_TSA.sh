...
mkdir -p $output_dir/zero_shot_logs
...
CUDA_VISIBLE_DEVICES=$gpu_id accelerate launch --main_process_port $port_no main.py \
    ...
    --root_results_dir ../../zero_shot_results/peft_with_ewc_${identity}_${epochs}_${importance}_enl_${encoder_ids_for_lang}_ent_${encoder_ids_for_task}_del_${decoder_ids_for_lang}_det_${decoder_ids_for_task} \
    --root_data_dir ../../decaNLP_final_dataset_with_verified_len \
    --logging_steps 100 \
    --do_eval \
    ...
    --do_zero_shot_inference  2>&1 | tee $output_dir/zero_shot_logs/$seq_no.log
